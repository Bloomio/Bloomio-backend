...
exports.default = function (request, response, next) {
  if (!request.headers.authorization) {
    return next(new _httpErrors2.default(400, 'AUTH - invalid request (!request.headers.authorization)'));
  }
  var base64AuthHeader = request.headers.authorization.split('Basic ')[1];
  if (!base64AuthHeader) {
    return next(new _httpErrors2.default(400, 'AUTH - invalid request (!base64AuthHeader)'));
  }
  ...
  if (!username || !password) {
    return next(new _httpErrors2.default(400, 'AUTH - invalid request (!username || !password)'));
  }
  return _account2.default.findOne({ username: username }).then(function (account) {
    if (!account) {
      return next(new _httpErrors2.default(400, 'AUTH - invalid request (!account)'));
    }
